...
GainAudioProcessorEditor::GainAudioProcessorEditor(GainAudioProcessor& p)
  ...
  titleComponent->sliderValueChanged(knob.get());
...
void GainAudioProcessorEditor::mouseDrag(const MouseEvent& event) {
  if (event.mods.isRightButtonDown()) {
    ...
    const float newX =
        p_x->convertTo0to1(posWhenStartedDragging.x + static_cast<float>(offset.x) / static_cast<float>(getWidth()));
    const float newY =
        p_y->convertTo0to1(posWhenStartedDragging.y + static_cast<float>(offset.y) / static_cast<float>(getHeight()));
...
void GainAudioProcessorEditor::mouseWheelMove(const MouseEvent&, const MouseWheelDetails& mouse_wheel_details) {
  const auto p_size = audioProcessor.apvts->getParameter(P_SIZE_ID);
  const auto sizeMultiplier =
      jlimit<float>(0.0f, 1000.0f, p_size->convertFrom0to1(p_size->getValue()) + 0.2f * mouse_wheel_details.deltaY);
